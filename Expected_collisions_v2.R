...
library(scales)
library(ggpubr)
# Resource: https://math.stackexchange.com/questions/35791/birthday-problem-expected-number-of-collisions

n <- 1:450 # population
# Expected number of people to share birthday in a population of size n
exp_collisions <- n*(1-(1-(1/365.25))^(n-1))

bd_collisions <- data.frame(
  n,
  exp_collisions
)

# Expected number of people sharing birthday for Ubiqum's team (n = 35)
bd_collisions$exp_collisions[bd_collisions$n==35] #3.11507

tail(bd_collisions, 1)
# n           exp_collisions
# 100         23.78448
# 250         123.7413
# 365         230.5398
...
# Line plot
ggplot(data = bd_collisions, aes(n, exp_collisions)) + 
